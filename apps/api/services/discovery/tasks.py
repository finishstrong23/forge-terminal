...
def sweep_stale_events():
    """
    Periodic task (every 5 min): bound the helius_events table on disk.

    1. DELETE rows older than EVENT_RETENTION_HOURS — at firehose volume
       their raw_data JSON is what fills the DB volume (it crashed Postgres
    ...
    try:
        ...
        archived = (
            db.query(HeliusEvent)
            .filter(HeliusEvent.processed.is_(False), HeliusEvent.received_at < archive_cutoff)
            .update(
                {
                    HeliusEvent.processed: True,
                    HeliusEvent.processed_at: now,
